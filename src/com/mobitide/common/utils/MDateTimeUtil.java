...
public class MDateTimeUtil {
    ...
    private final static String YEAR_MONTH_DAR_HOUR_MINUTE = "yyyy-MM-dd HH:mm";
    ...
    public static String getNewsTime(String timeStamp) {
        if (!MStringUtil.isOK(timeStamp)) {
            return "";
        }
        SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT);
        return sdf.format(Long.valueOf(timeStamp) * 1000);
    }

    public static String getAskTime(String timeStamp) {
        if (!MStringUtil.isOK(timeStamp)) {
            return "";
        }
     ...
     * @return
     */
    public static int cacDate() {
        Calendar calendar = Calendar.getInstance();
        int time = calendar.get(Calendar.MONTH);
        return time + 1;
    ...
    public static String getInforTime(String timeStamp) {

        SimpleDateFormat sdf = new SimpleDateFormat("MM-dd");
        String time;
        try {
            time = sdf.format(Long.valueOf(timeStamp) * 1000);
        } catch (Exception e) {
    ...
    public static String getFormatDate11(String timeStamp, String formate) {
     ...
     * @return
     */
    public static String getFormatDate12(String timeStamp, String formate) {
     ...
     * @return
     */
    protected static int cacDay(int month) {
        Calendar calendar = Calendar.getInstance();
        int year = calendar.get(Calendar.YEAR);
        MLogUtil.d("cac month = " + month);
    ...
    public static int maxContinuousDays(Date[][] startDateAndEndDate) {
        ...
        for (int i = 0; i < startDateAndEndDate.length - 1; i++) {
            for (int j = 0; j < startDateAndEndDate.length - i - 1; j++) {
                if (MDateTimeUtil.getDaysBetweenDates(startDateAndEndDate[j + 1][0], startDateAndEndDate[j][0]) > 0) {
        ...
        // break;
        // APPLOG.d(DateTimeUtil.getFormatDate(
        ...
        for (int i = 0; i < startDateAndEndDateNew.length; i++) {
            if (j >= startDateAndEndDate.length) break;

            startDateAndEndDateNew[i] = startDateAndEndDate[j];
            j++;
            while (j < startDateAndEndDate.length) {
                if (MDateTimeUtil.getDaysBetweenDates(startDateAndEndDateNew[i][1], startDateAndEndDate[j][0]) > 0) {
                    break;
                } else if (MDateTimeUtil.getDaysBetweenDates(startDateAndEndDateNew[i][1], startDateAndEndDate[j][1]) > 0) {
        ...
        // break;
        // APPLOG.d(DateTimeUtil.getFormatDate(startDateAndEndDateNew[i][0]) +
        // ","
        ...
        for (int i = 0; i < startDateAndEndDateNew.length - 1; i++) {
            ...
            int temDays = MDateTimeUtil.getDaysBetweenDates(curEndDate, nextStartDate);
            if (temDays > maxDays) {
                maxDays = temDays;
            }
        }
    ...
    public static int maxContinuousDays(String dateStr) {
        ...
        for (int i = 0; i < seDate.length; i++) {
            ...
            startDateAndEndDate[i][0] = MDateTimeUtil.getFormatDate(tempDate[0]);
            startDateAndEndDate[i][1] = MDateTimeUtil.getFormatDate(tempDate[1]);
        }
